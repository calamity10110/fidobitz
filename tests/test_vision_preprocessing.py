...
try:
    ...
    if isinstance(
        np, type(sys)
    ):  # It's a real module, usually Mocks aren't modules but MagicMock could act like one.
        pass
...
def test_roi():
    ...
    pre = VisionPreprocessor(
        {"resize": (224, 224), "normalize": False, "roi": (100, 100, 200, 200)}
    )
    out = pre.process(frame)
    assert out.shape == (224, 224, 3)
