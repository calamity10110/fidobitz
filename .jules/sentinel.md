...
## 2026-05-13 - Path Traversal via Improper Regex Validation
**Vulnerability:** HTTP endpoints reading `trace_id` and `name` and using them directly to format a zip file path or enrollment paths relied on `re.match` with a `$` anchor for validation (e.g., `re.match(r"^[a-zA-Z0-9_ -]+$")`). In Python, `re.match` can unexpectedly permit a trailing newline character (`\n`), circumventing the strict character boundaries and potentially leading to path traversal or downstream command injection issues.
**Learning:** `re.match` is insufficient for strict input validation because it succeeds even if the string ends with a newline, making it possible for attackers to bypass simple path checks by appending `\n` and injecting malicious content later.
**Prevention:** In Python, when using regular expressions for strict security input validation (e.g., preventing path traversal or HTTP header injection), use `re.fullmatch()` rather than `re.match()` with a `$` anchor. `re.fullmatch` strictly evaluates the entire string, leaving no room for trailing newlines.
