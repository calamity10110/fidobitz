...
class TelemetryHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _auth_ok(self, module, params):
    ...
    def _handle_snapshot(self, module, params):
    ...
    def _handle_download_slam_map(self, module):
    ...
    def _handle_download_support_bundle(self, module, params):
    ...
    def _handle_download_slam_trajectory(self, module):
    ...
    def _handle_dashboard(self, module):
    ...
    def do_GET(self):
        ...
        module = self.server.telemetry_module
        ...
        if not self._auth_ok(module, params):
            self._send_json({"error": "unauthorized"}, status=401)
            return

        if path == "/snapshot":
            self._handle_snapshot(module, params)
            return

        if path == "/download_slam_map":
            self._handle_download_slam_map(module)
            return

        if path == "/download_support_bundle":
            self._handle_download_support_bundle(module, params)
            return

        if path == "/download_slam_trajectory":
            self._handle_download_slam_trajectory(module)
            return

        if path == "/":
            self._handle_dashboard(module)
            return
