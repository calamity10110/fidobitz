...
class MappingModule(Module):
    ...
    def _ingest_into_grid(self, angles: dict, settings: dict, mapping_state: dict) -> None:
        ...
        for key, raw in angles.items():
            ...
            # Fast path for string representations of integer angles
            if key in trig_cache_str:
                cos_rad, sin_rad = trig_cache_str[key]
            else:
                try:
                    yaw = float(key)
                except Exception:
            ...
            x_cm = dist * cos_rad  # forward
            y_cm = dist * sin_rad  # left
