...
class MappingModule(Module):
    ...
    def _ingest_into_grid(
        ...
        # ⚡ Bolt: Attempt a direct zero-allocation fast path first using .get()
        # since keys from JSON are often already strings. Fall back to str()
        # casting only on a cache miss.
