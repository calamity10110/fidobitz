...
class SensorHealthModule(Module):
    ...
    def tick(self, context) -> None:
        ...
        warn = False
        # Ultrasonic
        us_stale = now - getattr(sensor, "timestamp", 0) > float(
            settings.get("ultrasonic_stale_s", 1.0)
        )
        if us_stale or not getattr(sensor, "distance_valid", True):
            warn = True
            details["ultrasonic"] = "stale" if us_stale else "invalid"
        # IMU
        imu_stale = now - getattr(sensor, "timestamp", 0) > float(
            settings.get("imu_stale_s", 1.0)
        )
        if imu_stale or not getattr(sensor, "imu_valid", True):
            warn = True
            details["imu"] = "stale" if imu_stale else "invalid"
        # Touch
        touch_stale = now - getattr(sensor, "timestamp", 0) > float(
            settings.get("touch_stale_s", 1.0)
        )
        if touch_stale or not getattr(sensor, "touch_valid", True):
            warn = True
            details["touch"] = "stale" if touch_stale else "invalid"
        # Sound
        sound_stale = now - getattr(sensor, "timestamp", 0) > float(
            settings.get("sound_stale_s", 1.0)
        )
        ...
        if warn:
            status = "warning"
        if settings.get("error_is_critical", True) and (
            not getattr(sensor, "distance_valid", True)
            or not getattr(sensor, "imu_valid", True)
        ):
            status = "error"
        # LED color
        ...
        if (
            self._last_status != status or now - self._last_log_ts > 10
        ) and settings.get("log_enabled", True):
